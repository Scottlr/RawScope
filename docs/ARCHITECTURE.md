...
The current Milestone 3 and 4B slice keeps this narrower: `rawscope-render` owns GPU scatter-density compute, GPU timeline-density compute, simple scatter and timeline presentation renderers, scatter-specific viewport math, testable brush geometry, CPU-side selected-region summaries/evidence for synthetic points, deterministic JSON/Markdown evidence artifact formatting, and a minimal brush overlay pass. Compute uploads synthetic point/event coordinates, bins counts with WGSL atomics, and can read counts back for correctness or setup diagnostics. Brush interaction separates in-progress screen-space drag rectangles from finalized data-space selections; summaries, evidence, overlay projection, and exported artifacts all use the finalized data-space selection as their source of truth. The native workbench renders scatter density counts with the same WGPU device/queue used by the window surface, then composites the projected brush rectangle after the density pass, avoiding a separate compute device in the visual path. `rawscope-workbench` translates winit mouse/keyboard events into viewport updates, brush updates, deterministic point-count presets, compact title-bar diagnostics, one-shot evidence logging, collision-safe local export file writes, and manifest append operations, then asks the scatter renderer to re-bin the visible range. Timeline density currently uses a startup `--demo timeline` path that generates deterministic synthetic events, computes GPU timeline-density counts, and renders a simple full-window density view with title-bar diagnostics. Timeline brushing, timeline row evidence, live mode switching, arbitrary timestamp normalization, and polished timeline UI remain deferred. `rawscope-gpu` owns both the reusable headless compute context for ignored correctness tests and the window surface context. GPU row-id preservation, row-level drilldown, file dialogs, and a table UI remain deferred until a later evidence-focused design.
