...
The current Milestone 3 slice keeps this narrower: `rawscope-render` owns GPU scatter-density compute, a simple presentation renderer, scatter-specific viewport math, testable brush geometry, CPU-side selected-region summaries/evidence for synthetic points, and a minimal brush overlay pass. Compute uploads synthetic point coordinates, bins counts with WGSL atomics, and can read counts back for correctness or setup diagnostics. Brush interaction separates in-progress screen-space drag rectangles from finalized data-space selections; summaries, evidence, and overlay projection all use the finalized data-space selection as their source of truth. The native workbench renders density counts with the same WGPU device/queue used by the window surface, then composites the projected brush rectangle after the density pass, avoiding a separate compute device in the visual path. `rawscope-workbench` translates winit mouse/keyboard events into viewport updates, brush updates, deterministic point-count presets, compact title-bar diagnostics, and one-shot evidence logging, then asks the renderer to re-bin the visible range. `rawscope-gpu` owns both the reusable headless compute context for ignored correctness tests and the window surface context. GPU row-id preservation, row-level drilldown, and a table UI remain deferred until a later evidence-focused design.
