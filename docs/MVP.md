...
- Status note: Milestone 5C adds the first data-quality primitive for local CSV work: a CPU-backed missingness heatmap slice built from retained source rows. The workbench can expose a `Missingness` surface for local datasets, aggregate trimmed-empty values by row bucket and column, and summarize selected cells back to counts, columns, and sorted row ids. This remains a correctness-first local slice, not a full data profiler, Parquet path, or GPU heatmap.
