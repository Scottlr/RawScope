...
- Status note: Milestone 5A adds the first real-data vertical slice: the native workbench can load local files with explicit column bindings for scatter density (`--x`/`--y`) and timeline density (`--time`/`--lane`). The slice began with CSV support, and T011 extends it with narrow Arrow/Parquet-backed chunked ingestion after dataset identity, retained source rows, evidence v2, and drilldown are stable. Scatter accepts integer or float-compatible x/y values; timeline accepts integer timestamps and string or integer lanes, mapping lanes deterministically. This does not add egui, Tauri, row tables, SQL/DataFusion, a dataframe abstraction, or new schema work.
...
- Status note: Milestone 5C adds the first data-quality primitive for local work: a CPU-backed missingness heatmap slice built from retained source rows. The workbench can expose a `Missingness` surface for local datasets, aggregate trimmed-empty values by row bucket and column, and summarize selected cells back to counts, columns, and sorted row ids. This remains a correctness-first local slice, not a full data profiler, broader file-import platform, or GPU heatmap.
...
- Status note: The workbench now exports scatter and timeline selections as local report bundles under `target/rawscope-exports/`. Each bundle includes source-aware v2 `evidence.json`, `evidence.md`, a bundle `manifest.json`, and a deterministic `visual-context.txt` placeholder that records the exact view configuration while native image capture remains deferred behind the documented readback/PNG design. v1 formatters and schema docs remain for compatibility.
